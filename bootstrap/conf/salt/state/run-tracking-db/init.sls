...
create_chunks_table:
  cmd.run:
    - user: postgres
    - name: psql -d germline_genotype_tracking -c "CREATE TABLE chunks (chunk_id serial PRIMARY KEY, chunk_location varchar(512), created_date timestamp, last_updated_date timestamp)"

create_runs_table:
  cmd.run:
    - user: postgres
    - name: psql -d germline_genotype_tracking -c "CREATE TABLE genotyping_runs (run_id serial PRIMARY KEY, chunk_id serial REFERENCES chunks(chunk_id), donor_index serial REFERENCES pcawg_samples(index), sample_id text NOT NULL, run_status integer NOT NULL, created_date timestamp, run_start_date timestamp, run_end_date timestamp, last_updated_date timestamp, run_error_code integer)"
