...
def parse_args():
    ...
    create_configs_parser.add_argument(
        "-a", "--analysis_id", help="ID of the analysis to run.", dest="analysis_id", required=True)
    create_configs_parser.add_argument(
        "-n", "--num_runs", help="Number of runs to create configurations for.", dest="num_runs", required=True, type=int)
    create_configs_parser.add_argument(
        "-t", "--tissue_type", help="Tumor or normal tissue", dest="tissue_type", choices=["tumor", "normal"], required=True)
    create_configs_parser.add_argument(
...
def get_available_samples(analysis_id, tissue_type, num_runs):
    ...
    current_runs = session.query(Configuration.config[("sample", " sample_id")].astext).\
        ...
        filter(
            and_(Analysis.analysis_id == analysis_id, AnalysisRun.run_status != 4))
...
def create_configs_command(args):
    ...
    available_samples, num_available_samples = get_available_samples(
        analysis_id, tissue_type, num_runs)
