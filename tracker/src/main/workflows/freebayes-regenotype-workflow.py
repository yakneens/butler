...
def set_ready(my_run):
    if my_run.run_status == 1:
        ...
        logger.error(
            "Attempting to put an In Progress run into Ready state, runID: %d", my_run.run_id)
...
def set_in_progress(my_run):
    if my_run.run_status != 0:
        ...
        logger.error("Wrong run status - %d, Only a Ready run can be put In Progress, runID: %d",
                     my_run.run_status, my_run.run_id)
        raise Exception()
    else:
        my_run.run_status = 1
...
def set_finished(my_run):
    if my_run.run_status != 1:
        ...
        logger.error("Wrong run status - %d, Only an In Progress run can be Finished, runID: %d",
                     my_run.run_status, my_run.run_id)
        raise Exception()
    else:
        my_run.run_status = 2
...
set_status = {"0": set_ready, "1": set_in_progress,
              "2": set_finished, "3": set_error}


def update_run_status(donor_index, sample_id, run_status):
    Base = automap_base()
    engine = create_engine(
        'postgresql://pcawg_admin:pcawg@postgresql.service.consul:5432/germline_genotype_tracking')
    ...
    my_run = session.query(GenotypingRun).filter(and_(
        GenotypingRun.donor_index == this_donor_index, GenotypingRun.sample_id == this_sample_id)).first()
    ...
    if not my_run:
        ...
        logger.info(
            "No Genotyping Run found for donor %d. Creating new Genotyping Run", my_run.donor_index)
    ...
    logger.info("Setting run status for donor %d to %d",
                my_run.donor_index, new_status)
...
def get_next_sample():
    ...
    next_sample = session.query(PCAWGSample.index, PCAWGSample.normal_wgs_alignment_gnos_id, SampleLocation.normal_sample_location, GenotypingRun.run_id).\
        ...
        and_(SampleLocation.normal_sample_location != None,
             or_(GenotypingRun.run_status == None, and_(
                 GenotypingRun.run_status != 1, GenotypingRun.run_status != 2))
             )).\
        first()
    ...
    logger.info("Got the next available sample Donor Index: %d, Sample ID: %s, Sample Location: %s, Genotyping Run: %d.",
                donor_index, sample_id, sample_location, my_run_id)
    ...
    logger.info(
        "Setting run status for donor %d to In Progress", my_run.donor_index)
...
def set_error():
    ...
    os.system(
        "/tmp/germline-regenotyper/scripts/update-sample-status.py {{ task_instance.xcom_pull(task_ids='reserve_sample')[0] }} {{ task_instance.xcom_pull(task_ids='reserve_sample')[1] }} 3")
...
def run_freebayes(**kwargs):
    ...
    logger.info("Got sample assignment from the sample reservation task: %d %s %s",
                donor_index, sample_id, sample_location)

    result_path_prefix = "/tmp/freebayes-regenotype/" + sample_id

    if (not os.path.isdir(result_path_prefix)):
        logger.info(
            "Results directory %s not present, creating.", result_path_prefix)
        os.makedirs(result_path_prefix)

    result_filename = result_path_prefix + "/" + \
        sample_id + "_regenotype_" + contig_name + ".vcf"
    ...
    logger.info(
        "About to invoke freebayes with command %s.", freebayes_command)
...
def compress_sample(result_filename):
    ...
    logger.info("About to compress sample %s. Using command: %s.",
                result_filename, compression_command)
...
def generate_tabix(compressed_filename):
    ...
    logger.info("About to generate tabix for %s. Using command: %s.",
                compressed_filename, tabix_command)
    ...
    try:
        ...
        if retcode != 0:
            ...
            raise Exception(
                "Tabix generation terminated by signal" + str(retcode))
        else:
...
def copy_result(donor_index, sample_id, contig_name):
    results_directory_command = "mkdir -p " + \
        results_base_path + "/" + sample_id
    # os.system(results_directory_command)
    try:
        retcode = call(results_directory_command, shell=True)
        if retcode != 0:
            logger.error(
                "Results directory creation terminated by signal %d.", retcode)
            raise Exception(
                "Results directory creation terminated by signal" + str(retcode))
    except OSError as e:
        logger.error("Results directory creation failed %s.", e)
        raise

    move_command = "rsync -a -v --remove-source-files /tmp/freebayes-regenotype/" + sample_id + "/" + \
        sample_id + "_regenotype_" + contig_name + ".vcf.gz* " + \
        results_base_path + "/" + sample_id + "/"
    logger.info("About to move results for %d %s to shared storage. Using command '%s'",
                donor_index, sample_id, move_command)
    ...
    try:
        ...
        if retcode != 0:
            ...
            raise Exception(
                "Results moving terminated by signal" + str(retcode))
        else:
