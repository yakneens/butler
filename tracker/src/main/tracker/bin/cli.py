...
def parse_args():
    ...
    create_workflow_parser.add_argument("-v", "--workflow_version", help="Version of the workflow to create", dest="workflow_version", required=True)
    create_workflow_parser.add_argument("-c", "--config_file_location", help="Path to a config file for this workflow.", dest="config_file_location", required=True)
    create_workflow_parser.set_defaults(func=create_workflow_command)
    
    launch_workflow_parser = sub_parsers.add_parser("launch-workflow", parents=[common_args_parser], conflict_handler='resolve')
    launch_workflow_parser.add_argument("-w", "--workflow_id", help="ID of the workflow to run", dest="workflow_id", required=True)
    launch_workflow_parser.add_argument("-a", "--analysis_id", help="ID of the analysis to run", dest="analysis_id", required=True)
    launch_workflow_parser.add_argument("-c", "--config_location", help="Path to a directory that contains analysis run configuration files. Each file will generate one analysis run.", dest="config_location", required=False)
    ...
    my_group.add_argument("-w", "--workflow_id", help="ID of the workflow to update.", dest="workflow_id")
    my_group.add_argument("-a", "--analysis_id", help="ID of the analysis to update.", dest="analysis_id")
